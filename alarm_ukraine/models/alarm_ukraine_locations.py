...
class UkraineLocations(models.Model):
    ...
    def get_alarm_status(self):
        ...
        key_cached_alert_levels = f"alarm_ukraine.cached_alert_levels_{regionID}"
        ...
        cached_levels = ICPSudo.get_param(key_cached_alert_levels, 'Не визначено')
        ...
        try:
            ...
            # Извлекаем первый элемент из списков, если они не пустые, иначе возвращаем пустой словарь/строку
            # alert_levels = result[0].get('activeAlerts', [])[0].get('activeAlertLevels', [])[0].get('reason')
            if has_alert:
            ...
            ICPSudo.set_param(key_cached_alert_levels, str(alert_levels))
